...
gradcam = '..\\gradcam_results\\repeat_'
mp = '..\\mp_results\\repeat_'
im_size = 224
...
for repeat in range(repeats):
    ...
    gradcam_boxes.append(get_predicted_boxes(curr_gc_fn, t))
    mp_boxes.append(get_predicted_boxes(curr_mp_fn, t))

im_boxes = {}
for im_fn in gradcam_boxes[0]:
    curr_gc_boxes = []
    curr_mp_boxes = []
    ...
    for repeat in range(repeats):
        ...
        result_box[0] += (gradcam_boxes[repeat][im_fn][0] + mp_boxes[repeat][im_fn][0])/(2)
        result_box[1] += (gradcam_boxes[repeat][im_fn][1] + mp_boxes[repeat][im_fn][1])/(2)
        result_box[2] += (gradcam_boxes[repeat][im_fn][2] + mp_boxes[repeat][im_fn][2])/(2)
        result_box[3] += (gradcam_boxes[repeat][im_fn][3] + mp_boxes[repeat][im_fn][3])/(2)
